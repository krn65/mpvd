...
### The following information is up-to-date as of June 22, 2020.
...
Both `.zip` files contain `4,752` source code files from all Mozilla products with a variety of file extensions. Only `2,056` of the total `2,746` unique Bug IDs are represented from the downloaded source code files. As stated earlier, this is due to unavailable data or other issues encountered while scraping or parsing the associated product security advisory data. Only Bugzilla entries that are public, have a status of `Closed`, and available attachments (table of revisions) are considered.
